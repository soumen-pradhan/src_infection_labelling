...
_legends = [Line2D([0], [0], marker='o', color='w', label='Source', markerfacecolor='r', markersize=15),
            Line2D([0], [0], marker='o', color='w', label='Observers',
...
def PTVA(G, observers, Ka, sigma2, mn):
    ...
    for s in list(G.nodes()):
        ...
        score = np.dot(np.dot(mu_s.T, np.linalg.inv(delta_s)),
                       d - (0.5) * mu_s)
        likelihood[s] = score

    sortedLikelihood = sorted(
        likelihood.items(), key=lambda x: x[1], reverse=True)
    count = 0
